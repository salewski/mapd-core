...
ResultPtr Executor::executeWorkUnit(int32_t* error_code,
  ...
  const auto ra_exe_unit = addDeletedColumnFilter(ra_exe_unit_in);
...
RelAlgExecutionUnit Executor::addDeletedColumnFilter(const RelAlgExecutionUnit& ra_exe_unit) {
  ...
  for (const auto& input_table : ra_exe_unit_with_deleted.input_descs) {
    ...
    ra_exe_unit_with_deleted.quals.push_back(makeExpr<Analyzer::UOper>(
        deleted_cd->columnType,
        false,
        kNOT,
        makeExpr<Analyzer::ColumnVar>(
            deleted_cd->columnType, input_table.getTableId(), deleted_cd->columnId, input_table.getNestLevel())));
  }
